...
class MYConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        super().__init__()
        self.conv = ODConv2d(c1, c2, k, s, padding=autopad(k, p, d), groups=g, dilation=d)
...
class ODSPPF(nn.Module):
    ...
    def __init__(self, c1, c2, k=5):  
        ...
        self.cv3 = MYConv(c_, c_, 3, 1)
        self.cv4 = MYConv(c_, c1, 1, 1)
        self.m = nn.ModuleList([nn.MaxPool2d(kernel_size=x, stride=1, padding=x//2) for x in k])
        self.cv5 = MYConv(4*c1, c2, 1, 1)
...
class BertBlock(nn.Module): # 融合 Bert 模块
    def __init__(self, cc, reduction=16):
        # 定义 Bert 相关操作
        super(BertBlock, self).__init__()
        self.path = '/repository7403/zhouxuefeng/ultralytics-ma/features.npy'
        self.document_features = np.load(self.path)
        self.mlp = nn.Sequential(
            nn.Linear(self.document_features.shape[1], 512),
            nn.SiLU(),
            nn.Linear(512, 256),
            nn.SiLU(),
            nn.Linear(256, 128),
        ...
        self.rnns = nn.Sequential(
            nn.GRU(self.embed_dim, self.embed_dim, 2),
            nn.Linear(self.embed_dim, self.embed_dim),
            nn.SiLU(),
        ).to(DEVICE)

        self.confusion_attention = SElayer(384, 4)
        self.one_conv = nn.Conv2d(384, 384//2, kernel_size=1, stride=1)

        
    def forward(self, x): # x 是图像信息
       
        document_vectors = torch.tensor(self.document_features, dtype=x.dtype).to(DEVICE)
        # print(f'{document_vectors.dtype}')
        for lay in self.mlp:
          
            document_vectors = lay(document_vectors)
       
        if(x.shape[0]!=16 or x.shape[1]!=192 or x.shape[2]!=80 or x.shape[3]!=80):
            return x
      
        query = x
        query = query.view(x.shape[1], x.shape[0], x.shape[2]*x.shape[3])
        key = document_vectors
        key = key.view(1, 1, document_vectors.shape[0]*document_vectors.shape[1]).repeat(x.shape[1], x.shape[0], 1)
        value = torch.tensor(key)
        output, _ = self.multihead_attn(query, key, value)

        # GRU 模块
        for i, lay in enumerate(self.rnns):
            if(i==0):
                output, _=lay(output)
            else:
                output= lay(output)

        output = output.view(x.shape[1], x.shape[0], x.shape[2], x.shape[3])  # 将output的batch_size和序列长度维度拆分回原来的大小
        output = output.transpose(1, 0)
        # return output
        # print(output.shape)

        # cat 模块
        confusion_features = torch.cat([x, output], dim=1)
        
       

        # SE-layer 模块
        confusion_features = self.confusion_attention(confusion_features)
        res = self.one_conv(confusion_features)
